...
import { addItemToCart, decreaseQuantity, increaseQuantity, deleteFromCart } from "../../Store/cartStore";

const MenuCategory = ({ title, items, isHorizontal }) => {
  const dispatch = useDispatch();
  const [cartItems, setCartItems] = useState([]); // Локальное состояние для корзины
    ...
    <div>
      ...
      <div className="menuCategory">
        {items.slice(0, visibleItems).map((item) => {
          ...
          return (
            <div key={item.id} className={`menuItem ${isHorizontal ? "menuItemHorizontal" : ""}`}>
              ...
              <div>
                ...
                <h3>${item.price}</h3>
                {!quantity ? (
                  <button className="filleadButton" onClick={() => handleAddToCart(item)}>
                    Add To Cart
                  </button>
                ) : (
                  <div className="filleadButton">
                    <button onClick={() => handleDecrease({ id: item.id, quantity })}>-</button>
                    <span>{quantity}</span>
                    <button onClick={() => handleIncrease({ id: item.id, quantity })}>+</button>
                  </div>
                )}
              </div>
            </div>
...
const MenuCard = ({ isHorizontal }) => {
  ...
  return (
    <div className="menuCard">
      <MenuCategory title="Starters" items={starters} isHorizontal={isHorizontal} />
      <MenuCategory title="Main Dishes" items={mains} isHorizontal={isHorizontal} />
      <MenuCategory title="Drinks" items={drinks} isHorizontal={isHorizontal} />
      <MenuCategory title="Desserts" items={desserts} isHorizontal={isHorizontal} />
    </div>
  );
};
